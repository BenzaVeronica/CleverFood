import { Icon, IconProps } from '@chakra-ui/react';

interface Props extends IconProps {
    isActive?: boolean;
    _secondaryColor?: string;
}
export const IconHomeCircle = ({
    isActive = false,
    _secondaryColor = 'black',
    ...props
}: Props) => (
    // const [color] = useToken("colors", [secondaryColor]);
    <Icon viewBox='0 0 40 40' fill='none' {...props}>
        {isActive && <rect width='40' height='40' rx='20' fill='black' />}
        <path
            fillRule='evenodd'
            clipRule='evenodd'
            d='M14 25.5V19H15V25.5C15 25.6326 15.0527 25.7598 15.1464 25.8536C15.2402 25.9473 15.3674 26 15.5 26H24.5C24.6326 26 24.7598 25.9473 24.8536 25.8536C24.9473 25.7598 25 25.6326 25 25.5V19H26V25.5C26 25.8978 25.842 26.2794 25.5607 26.5607C25.2794 26.842 24.8978 27 24.5 27H15.5C15.1022 27 14.7206 26.842 14.4393 26.5607C14.158 26.2794 14 25.8978 14 25.5ZM25 14.5V18L23 16V14.5C23 14.3674 23.0527 14.2402 23.1464 14.1464C23.2402 14.0527 23.3674 14 23.5 14H24.5C24.6326 14 24.7598 14.0527 24.8536 14.1464C24.9473 14.2402 25 14.3674 25 14.5Z'
            fill={`${isActive ? '#FFFFD3' : 'black'}`}
        />
        <path
            fillRule='evenodd'
            clipRule='evenodd'
            d='M19.293 13.5C19.4805 13.3125 19.7349 13.2072 20 13.2072C20.2652 13.2072 20.5195 13.3125 20.707 13.5L27.354 20.146C27.4479 20.2399 27.5007 20.3672 27.5007 20.5C27.5007 20.6328 27.4479 20.7601 27.354 20.854C27.2601 20.9479 27.1328 21.0006 27 21.0006C26.8672 21.0006 26.7399 20.9479 26.646 20.854L20 14.207L13.354 20.854C13.2601 20.9479 13.1328 21.0006 13 21.0006C12.8672 21.0006 12.7399 20.9479 12.646 20.854C12.5521 20.7601 12.4994 20.6328 12.4994 20.5C12.4994 20.3672 12.5521 20.2399 12.646 20.146L19.293 13.5Z'
            fill={`${isActive ? '#FFFFD3' : 'black'}`}
        />
    </Icon>
);
